...
def clean_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Keep rows where student name is available.
    cleaned = cleaned.dropna(subset=["name_of_student"]).reset_index(drop=True)

    # Fill missing numeric values by median to keep maximum rows.
    for col in NUMERIC_COLUMNS:
...
def main() -> None:
    ...
    parser.add_argument(
        "--input",
        default="all_year.xlsx",
        help="Path to source Excel file (default: all_year.xlsx)",
    )
    parser.add_argument(
        "--output",
